...
Bits hammingEncode(const Bits& dataBits) {
    ...
    Bits encodedBits;
    encodedBits.reserve(dataBits.size() / 4 * 7);
    ...
    for (std::size_t i = 0; i < dataBits.size(); i += 4) {
        Bits codeword = hammingEncodeBlock(
            ...
            dataBits[i + 2],
            dataBits[i + 3]);
        encodedBits.insert(encodedBits.end(), codeword.begin(), codeword.end());
    }
...
void applyNoise(Carrier& carrier, int noisePercent, unsigned int seed) {
    const std::size_t attackCount =
        carrier.size() * static_cast<std::size_t>(noisePercent) / 100;
    const std::vector<std::size_t> positions =
        generateRandomPositions(carrier.size(), attackCount, seed);
...
ExperimentResult runExperiment(
    ...
    if (noisePercent > 0) {
        applyNoise(attackedCarrier, noisePercent, embedSeed + 1009U * noisePercent);
    }
...
void printStats(const DecodeStats& stats) {
    std::cout << "总码字数: " << stats.totalCodewords << '\n'
              << "无错误码字数: " << stats.cleanCodewords << '\n'
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        const unsigned int seed =
            argc > 2 ? static_cast<unsigned int>(std::stoul(argv[2])) : 202405U;
        const std::size_t carrierSize =
            argc > 3 ? static_cast<std::size_t>(std::stoull(argv[3])) : 100000U;
